...
describe.skipIf(!DATABASE_URL)('Phase 5 accounting chaos certification', () => {
  ...
  it('survives create → correct → locked second correction → void without residue or lost history', async () => {
    ...
    const locked = await request(`/api/entries/${id}`, {
  ...
  it('serializes concurrent corrections without duplicate active accounting effects', async () => {
    ...
    expect([a, b].filter((result) => result.response.status === 200).length).toBe(1);
  ...
  it('survives correction racing void and always finishes voided with zero active effects', async () => {
    ...
    expect([200, 404, 409]).toContain(correction.response.status);
    ...
    const correctionRevisions = Number((await db<{ n: string }>(
      `SELECT count(*) AS n FROM entry_revisions WHERE entry_id = $1 AND revision_type = 'correction'`, [id],
    ))[0].n);
    expect(correctionRevisions).toBe(correction.response.status === 200 ? 1 : 0);
