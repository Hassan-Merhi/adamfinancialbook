...
export async function createSecuritySession(input: {
  ...
  await query(
    `INSERT INTO user_sessions
     ...
     VALUES ($1,$2,$3,$4,$5,$6)`,
    [id, input.userId, input.tokenVersion, expiresAt, input.ip ? sha256(input.ip) : null, cleanAgent(input.userAgent)],
  );
  return { id, expiresAt, maxAgeSeconds };
}

export async function validateSecuritySession(id: string, userId: string, tokenVersion: number): Promise<SecuritySession | null> {
...
export async function hasRecentAuthentication(sessionIdValue?: string | null): Promise<boolean> {
  ...
  const rows = await query<{ ok: boolean }>(
    `SELECT EXISTS(
     ...
     ) AS ok`, [sessionIdValue, RECENT_AUTH_SECONDS],
  );
  return !!rows[0]?.ok;
}

export async function revokeSecuritySession(userId: string, sessionIdValue: string, revokedBy: string): Promise<boolean> {
  const rows = await query(
    `UPDATE user_sessions
        SET revoked_at = COALESCE(revoked_at, now()), revoked_by = COALESCE(revoked_by, $3)
      WHERE id = $1 AND user_id = $2 AND revoked_at IS NULL
      RETURNING id`, [sessionIdValue, userId, revokedBy],
  );
  return !!rows[0];
}

export async function revokeUserSessions(userId: string, revokedBy: string, exceptSessionId?: string | null): Promise<number> {
  const rows = await query(
    `UPDATE user_sessions
        SET revoked_at = COALESCE(revoked_at, now()), revoked_by = COALESCE(revoked_by, $2)
      WHERE user_id = $1 AND revoked_at IS NULL
        AND ($3::text IS NULL OR id <> $3)
      RETURNING id`, [userId, revokedBy, exceptSessionId ?? null],
  );
  return rows.length;
}
...
function throttleKey(ip: string | undefined, login: string): string {
  return sha256(`${ip ?? 'unknown'}|${login.trim().toLowerCase()}`);
}

export async function loginThrottleStatus(ip: string | undefined, login: string): Promise<{ locked: boolean; retryAfterSeconds: number }> {
  const key = throttleKey(ip, login);
  const rows = await query<{ locked_until: Date | null }>('SELECT locked_until FROM login_throttle WHERE key_hash = $1', [key]);
...
export async function noteLoginFailure(ip: string | undefined, login: string): Promise<{ locked: boolean; justLocked: boolean; retryAfterSeconds: number }> {
  const key = throttleKey(ip, login);
  ...
  try {
    ...
    await client.query(
      `INSERT INTO login_throttle (key_hash, failure_count, window_started_at, updated_at)
       VALUES ($1,0,now(),now()) ON CONFLICT (key_hash) DO NOTHING`, [key],
    );
    const result = await client.query<any>(
      `SELECT failure_count, window_started_at, locked_until
         FROM login_throttle WHERE key_hash = $1 FOR UPDATE`, [key],
    );
    ...
    const count = resetWindow ? 1 : Number(row.failure_count) + 1;
    ...
    await client.query(
      `UPDATE login_throttle
          SET failure_count = $2,
              ...
              locked_until = $4,
              updated_at = now()
        WHERE key_hash = $1`, [key, count, resetWindow, lockedUntil],
    );
    await client.query('COMMIT');
    return { locked: justLocked, justLocked, retryAfterSeconds: justLocked ? Math.ceil(LOGIN_LOCK_MS / 1000) : 0 };
  } catch (error) {
...
export async function clearLoginFailures(ip: string | undefined, login: string): Promise<void> {
  await query('DELETE FROM login_throttle WHERE key_hash = $1', [throttleKey(ip, login)]);
}
...
function totpAt(secretValue: string, timeMs: number): string {
  ...
  const number = ((((digest[offset] & 0x7f) << 24) | ((digest[offset + 1] & 0xff) << 16) | ((digest[offset + 2] & 0xff) << 8) | (digest[offset + 3] & 0xff))) % 1_000_000;
...
export function totpUri(username: string, secretValue: string): string {
  const issuer = 'Adam Financial Book';
  const label = `${issuer}:${username}`;
  const params = new URLSearchParams({ secret: secretValue, issuer, algorithm: 'SHA1', digits: '6', period: '30' });
