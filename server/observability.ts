  ...
  res.on('finish', () => {
    ...
    if (res.statusCode >= 500) {
      ...
      if (requestMetrics.responses5xx % 3 === 0) {
        fireOperationalAlert('http.repeated_5xx', { ...detail, responses5xxSinceStart: requestMetrics.responses5xx }, 'critical');
      }
...
export async function recordOperationalEvent(
  ...
  try {
    ...
    console.error(JSON.stringify({
      ts: new Date().toISOString(), severity: 'error', event: 'operational_event.persist.failed',
      originalEvent: event, error: error instanceof Error ? error.message : String(error),
    }));
  }
}
...
export async function latestBackupStatus() {
  const rows = await query<{
    id: string; started_at: Date; completed_at: Date | null; status: 'running' | 'success' | 'failed';
    destination: string; bytes: string | number | null; checksum: string | null;
    migration_version: string | number | null; encrypted: boolean; error: string | null;
  }>(
  ...
  const ageHours = completedAt ? Math.round(((Date.now() - Date.parse(completedAt)) / 3_600_000) * 10) / 10 : null;
  return {
    id: row.id,
    startedAt: new Date(row.started_at).toISOString(), completedAt, status: row.status,
    destination: row.destination, bytes: row.bytes === null ? null : Number(row.bytes),
    checksum: row.checksum?.trim() ?? null,
    migrationVersion: row.migration_version === null ? null : Number(row.migration_version),
    encrypted: row.encrypted, error: row.error, ageHours,
    stale: row.status === 'success' && ageHours !== null && ageHours > Number(process.env.BACKUP_STALE_HOURS ?? 30),
  };
}
...
export async function operationsStatus() {
  ...
  const [ready, migrations, backup, recentEvents, failedLoginRows] = await Promise.all([
    readiness(),
    getMigrationStatus().catch(() => null),
    latestBackupStatus(),
    query<{ severity: string; n: number }>(
      `SELECT severity, count(*)::int AS n FROM operational_events
        WHERE created_at >= now() - interval '24 hours' GROUP BY severity`,
    ).catch(() => []),
    query<{ n: number }>(
      `SELECT count(*)::int AS n FROM audit
        WHERE at >= now() - interval '24 hours'
          AND action IN ('sign-in refused','sign-in locked')`,
    ).catch(() => []),
  ]);
  ...
  return {
    ...
    database: {
      status: ready.database, latencyMs: databaseLatencyMs,
      pool: { total: pool.totalCount, idle: pool.idleCount, waiting: pool.waitingCount },
    },
    migrations: migrations
      ? { current: migrations.current, latest: migrations.latest, pending: migrations.pending }
    ...
    security: { failedLogins24h: Number(failedLoginRows[0]?.n ?? 0) },
    memory: {
    ...
    events24h: {
      info: bySeverity.info ?? 0, warn: bySeverity.warn ?? 0,
      error: bySeverity.error ?? 0, critical: bySeverity.critical ?? 0,
    },
  };
}
...
export async function recentOperationalEvents(limit = 100) {
  ...
  return query<{
    id: string; created_at: Date; severity: OperationalSeverity; event: string;
    request_id: string | null; detail: Record<string, unknown>;
  }>(
    `SELECT id, created_at, severity, event, request_id, detail
       FROM operational_events ORDER BY created_at DESC LIMIT $1`,
    [safeLimit],
  ).then((rows) => rows.map((row) => ({
    id: row.id, createdAt: new Date(row.created_at).toISOString(), severity: row.severity,
    event: row.event, requestId: row.request_id, detail: row.detail,
  })));
}
