...
import { ownerOnly, requireLogin, userCount } from './auth.js';
...
// Authentication, sessions, MFA, login throttling and access lifecycle all live
// in auth.ts/security.ts. Keeping one security entrypoint prevents an older route
// from silently bypassing newer controls.
...
// must also carry a header a form cannot set. Security routes intercepted by
// requireLogin apply the same check before responding.
