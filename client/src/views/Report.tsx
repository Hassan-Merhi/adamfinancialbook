/** The whole day on one screen, with any past day rebuilt from ledger effects. */
  ...
  return (
    <section className="report-page">
      <div className="dhead report-head">
        ...
        <span className="chip">{day.length} entr{day.length === 1 ? 'y' : 'ies'}</span>
      </div>
      ...
      <Card title={`Cash at end of ${shortDate(date)}`} aside={`${businesses.length} business${businesses.length === 1 ? '' : 'es'}`}>
        {businesses.map((b) => (
          <div className="row" key={b.id}>
            <span className="main"><b>{b.name}</b><small>{book.accounts.filter((a) => a.businessId === b.id).map((a) => `${a.name} ${money(accountBalance(book, a.id, date))}`).join(' · ') || 'no accounts'}</small></span>
      ...
      <div className="report-flow-grid">
        <Card title="Received" aside={received.length ? money(receivedTotal) : undefined}>
          {received.length === 0 ? <Empty>Nothing received this day.</Empty> : received.map((e) => (
            <div className="row" key={e.id}><span className="main"><b>{book.projects.find((p) => p.id === e.projectId)?.name ?? e.purpose}</b><small>{book.accounts.find((a) => a.id === e.accountId)?.name}</small></span><span className="val num pos">{signed(e.amount)}</span></div>
          ))}
        </Card>
        <Card title="Spent" aside={spent.length ? money(spentTotal) : undefined}>
          {spent.length === 0 ? <Empty>Nothing spent this day.</Empty> : spent.map((e) => (
            <div className="row" key={e.id}><span className="main"><b>{e.purpose}</b><small>{book.accounts.find((a) => a.id === e.accountId)?.name}</small></span><span className="val num neg">{signed(-e.amount)}</span></div>
          ))}
        </Card>
      </div>

      {credit.length > 0 && <Card title="Taken on credit" aside="not paid yet">{credit.map((e) => <div className="row" key={e.id}><span className="main"><b>{e.purpose}</b><small>{book.people.find((p) => p.id === e.personId)?.name}</small></span><span className="val num neg">{money(-e.amount)}</span></div>)}</Card>}
      {moved.length > 0 && <Card title="Moved between accounts" aside="not spending">{moved.map((e) => <div className="row" key={e.id}><span className="main"><b>{book.accounts.find((a) => a.id === e.accountId)?.name} → {book.accounts.find((a) => a.id === e.toAccountId)?.name}</b><small>{e.purpose}</small></span><span className="val num">{money(e.amount)}</span></div>)}</Card>}
      ...
      <Card title="Reminders" aside="promises, not movements">
        {book.reminders.length === 0 ? <Empty>Nothing pending.</Empty> : book.reminders.map((r) => (
          <div className="row" key={r.id}><span className="main"><b>{r.what}</b><small>{[book.accounts.find((a) => a.id === r.accountId)?.name, r.note].filter(Boolean).join(' · ')}</small></span><span className="val num">{money(r.amount)}<small><button className="linkbtn" onClick={() => run(() => api.clearReminder(r.id), 'Reminder cleared.')}>clear</button></small></span></div>
        ))}
      </Card>
    </section>
  );
