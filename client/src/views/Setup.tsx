...
export default function Setup({ book, run, reload, say, onQueued }: {
  ...
  const confirmedAccounts = book.accounts.filter((item) => !pendingIds.has(item.id));
  const first = confirmedBusinesses[0]?.id ?? '';
    ...
    <section className="setup-page">
      ...
      <div className="card setup-card" id="setup-accounts">
        ...
        <div className="form setup-form">
          ...
          <button className="btn" disabled={!acc.name.trim()} onClick={() => {
            void keepSetup(() => outbox.setup({ setupType: 'account', name: acc.name.trim(), businessId: null, opening: Number(acc.opening) || 0 }), 'Account created.');
      ...
      <div className="card setup-card" id="setup-projects">
        ...
        <div className="form setup-form">
          ...
          <div className="f"><label>Business</label><select value={prj.businessId || first} onChange={(e) => setPrj({ ...prj, businessId: e.target.value })}>{confirmedBusinesses.map((b) => <option key={b.id} value={b.id}>{b.name}</option>)}</select></div>
          ...
          <button className="btn" disabled={!prj.name.trim() || !confirmedBusinesses.length} onClick={() => {
      ...
      <div className="card setup-card" id="setup-people">
        ...
        <div className="form setup-form">
          ...
          <div className="f"><label>Business</label><select value={per.businessId || first} onChange={(e) => setPer({ ...per, businessId: e.target.value })}>{confirmedBusinesses.map((b) => <option key={b.id} value={b.id}>{b.name}</option>)}</select></div>
          ...
          <button className="btn" disabled={!per.name.trim() || !confirmedBusinesses.length} onClick={() => {
      ...
      <div className="card setup-card" id="setup-reminders">
        ...
        <div className="form setup-form">
          ...
          <div className="f"><label>Account</label><select value={rem.accountId} onChange={(e) => setRem({ ...rem, accountId: e.target.value })}><option value="">Not decided</option>{confirmedAccounts.map((a) => <option key={a.id} value={a.id}>{a.name}</option>)}</select></div>
