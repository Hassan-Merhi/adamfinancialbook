...
async function send<T>(path: string, method: string, body?: unknown): Promise<T> {
  ...
  if (!res.ok) {
    ...
    throw new ApiError(said ? `${said}` : `Request failed (${res.status})`, res.status, data?.code);
  }
  return data as T;
}
...
export const api = {
  me: () => send<Me>('/me', 'GET'),
  login: (email: string, password: string, totp?: string) => send<Me>('/login', 'POST', { email, password, totp }),
